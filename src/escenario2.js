...
const nutresaa = [
 ...
 export default function Escenario2( {setPortaec2,setPortaBan2, setPortaCem2,setPortaNut2, setEcova, setBanva,setCemva,setNutva, cantiEco,cantiBan,cantiCem,cantiNut,  setEcova2,setBanva2,setCemva2,setNutva2, cantiEco2, cantiBan2,cantiCem2,cantiNut2  } ) { // aqui estoy recibiendo las props enviadas desde el parent App
  ...
  const precioEco = ((Ecoprice) + (Ecopriceg) + (Ecopricei)) / 3  ; 
  const precioBan = ((Banprice ) + (Banpriceg) + (Banpricei)) / 3  ; 
  const precioCem = ((Cemprice) + (Cempriceg) + (Cempricei)) / 3  ; 
  const precioNut = ((Nutreprice) + (Nutrepriceg) + (Nutrepricei)) / 3  ; 
  ...
  setPortaec2(precioEco) // aqui estoy dandole valor a este hook para que tome el valor para el dinero disponible
  

  setBanva( precioBan * cantiBan)
  setBanva2( precioBan * cantiBan2)
  setPortaBan2( precioBan)

  setCemva( precioCem * cantiCem)
  setCemva2( precioCem * cantiCem2)
  setPortaCem2( precioCem)

  setNutva( precioNut * cantiNut)
  setNutva2( precioNut * cantiNut2)
  setPortaNut2( precioNut)



  const previo = data[0];
  const noticias = data[1] // segundo escenario entonces pongo los datos [1] del objeto que nace de la array al azar
